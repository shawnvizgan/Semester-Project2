...
new_data <- tibble(Noise_type = rep(c("No noise","Interval Noise","Continuous Noise"), each=2),
                    Subjects = as.factor(rep(c(1,2),3)),
                    RXtime = c(300,600,500,300,400,600))
                    
                    
ggplot(fake_data, aes(x=Noise_type, y=RXtime, fill=Subjects))+
  geom_bar(stat='identity', position="dodge")
